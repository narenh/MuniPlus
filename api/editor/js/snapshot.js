...
function reportHtml(r) {
  ...
  <div class="drift">
    ...
    ${group('What it does to the curation', [
      sect('dead-stops', 'Stops that would go dead', d.deadStops, deadStop, true),
...
const stopAdded = s => item(`${code(s.stop)} ${esc(s.name)}`,
  ringAt([{ at: [s.lon, s.lat], code: upstream(s.stop) }]));

const stopRemoved = s => item(`${code(s.stop)} ${esc(s.name)} <span class="dim">· ${where(s.stop)}</span>`,
  ringAt([{ at: [s.lon, s.lat], code: upstream(s.stop) }]));

const stopMoved = s => item(`${code(s.stop)} ${esc(s.name)} <span class="dim">· ${Math.round(s.metres)} m · ${where(s.stop)}</span>`,
  ringAt([
    { at: [s.oldLon, s.oldLat], code: `${upstream(s.stop)} was` },
    { at: [s.lon, s.lat], code: upstream(s.stop) },
  ]));

const stopRenamed = s => item(`${code(s.stop)} ${esc(s.oldName)} → <b>${esc(s.name)}</b>`,
  stopPos(s.stop) ? () => { select(null, null); flyTo(stopPos(s.stop), 17); spotlight(s.stop); } : null);
...
// A dead stop may be an extra stop of a platform whose primary lives on, so the
// primary is named when it is not the stop itself.
const deadStop = d => item(`${code(d.stop)}${d.platform !== d.stop ? ` <span class="dim">of ${code(d.platform)}</span>` : ''} ${esc(d.stationName)} <span class="dim">· ${esc(d.heading)}</span>`,
  stationById(d.station) ? () => { select(d.station, d.platform); flyToStation(d.station); } : null);

const deadStation = d => item(`${esc(d.name)} <code>${esc(d.station)}</code> <span class="dim">· ${d.stops.map(p => esc(upstream(p))).join(' ')}</span>`,
