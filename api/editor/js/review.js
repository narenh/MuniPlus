...
import {
  store, edit, select, esc, upstream, lineById, stationById, platformsOf, ownerOf,
...
function stopDone(u) {
  const sid = ownerOf(u.platform);
  if (sid) return `Assigned to ${esc(stationById(sid)?.name || sid)}`;
  if (store.curation?.ignored?.[u.platform]) return 'Ignored';
  return null;
}

function platformDone(d) {
  const st = stationById(d.station);
  if (!st) return 'Station deleted';
  if (!platformsOf(st).some(p => p.id === d.platform)) return 'Removed';
  return null;
}

const stationDone = d => (stationById(d.station) ? null : 'Deleted');

/** A dead station's platforms are also dead platforms (the contract lists them
 *  in both). They are dealt with by deleting the station, so they are shown
 *  there and not twice. */
function loosePlatforms() {
  const dead = new Set((review?.deadStations || []).map(d => d.station));
  return (review?.deadPlatforms || []).filter(d => !dead.has(d.station));
}

export function pendingCount() {
  if (!review) return null;
  return review.unassigned.filter(u => !stopDone(u)).length
    + loosePlatforms().filter(d => !platformDone(d)).length
...
const headingOf = u => draftOf(u.platform).heading ?? u.proposal.heading ?? '';
...
export function renderReview() {
  ...
  $('review-chip').title = loadError ? `The review could not be loaded: ${loadError}`
    : `${n ?? '…'} to review: unassigned 511 stops, and platforms and stations 511 dropped (Q)`;
...
function queueHtml() {
  ...
  return `${err}${stale}
    ...
    ${section('dead-platforms', 'Platforms 511 dropped', loose.length,
      'Still in a station, but 511 no longer lists them.',
      capped(loose, platformRow))}
...
function stopRow(u) {
  ...
  <div class="rv-item ${done ? 'done' : ''} ${exp ? 'open' : ''}" data-row="stop" data-pid="${esc(pid)}">
    <div class="rv-row1">
      ...
      <span class="rv-name">${esc(u.stopName)}</span>
    </div>
...
function platformRow(d) {
  const done = platformDone(d);
  const st = stationById(d.station);
  const only = st && platformsOf(st).length === 1;
  return `
  <div class="rv-item ${done ? 'done' : ''}" data-row="platform" data-pid="${esc(d.platform)}" data-sid="${esc(d.station)}">
    <div class="rv-row1">
      <span class="rv-code mono" title="${esc(d.platform)}">${esc(upstream(d.platform))}</span>
    ...
    ${done
      ? `<div class="rv-done">✓ ${done} <span>· unsaved</span></div>`
      : `<div class="rv-actions">
          <button class="btn small" data-act="remove-platform" ${only ? 'disabled title="Its station has no other platform: delete the station instead"' : ''}>Remove from station</button>
        </div>`}
  </div>`;
}
...
function stationRow(d) {
  ...
  <div class="rv-item ${done ? 'done' : ''} ${exp ? 'open' : ''}" data-row="station" data-sid="${esc(d.station)}">
    ...
    <div class="rv-reason">Platforms ${d.platforms.map(p => `<code>${esc(upstream(p))}</code>`).join(' ')}, none in 511</div>
...
const stopOf = pid => review?.unassigned.find(u => u.platform === pid);
...
function goTo(row) {
  ...
  if (kind === 'stop') {
    ...
    spotlight(u.platform);
    return;
  }
  const sid = row.dataset.sid;
  if (!stationById(sid)) return;
  select(sid, kind === 'platform' ? row.dataset.pid : null);
  flyToStation(sid);
...
function act(what, row, button) {
  ...
  if (what === 'remove-platform') removePlatform(row.dataset.sid, row.dataset.pid);
...
function stopAction(what, u, row) {
  ...
  if (what === 'other') {
    ...
    openPalette('station', sid => { if (sid) assign(u, sid, heading); }, {
      near: [u.lon, u.lat],
      placeholder: `Station for ${upstream(pid)} ${u.stopName}, nearest first…`,
    });
  }
...
function assign(u, sid, heading) {
  ...
  if (!st) return;
  const ok = edit(`Assign ${upstream(u.platform)} to ${st.name}`, c => {
    c.stations.stations[sid].platforms.push({ id: u.platform, heading });
  });
  if (ok) { expanded = null; hint(`${upstream(u.platform)} → ${st.name}`); }
}
...
function removePlatform(sid, pid) {
  const st = stationById(sid);
  if (!st) return;
  if (platformsOf(st).length === 1) { hint('A station must keep at least one platform'); return; }
  edit(`Remove platform ${upstream(pid)} from ${st.name}`, c => {
    const s = c.stations.stations[sid];
